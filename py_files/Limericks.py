...
class Limerick_Generate:
    ...
    def gen_line_with_template(self, prompt, template):
        ...
        new_line : array
            The line generated by BGT that satisfies the template POS restrictions
        """
        # score_model(input=["current", "new_line"])
        # return
        ...
        for i in range(10):
            ...
            for POS in template[0]:
                sentence.append(random.choice(word_dict[POS]))
            curr = sentence[0]
            probability = 0
            for word in sentence[1:]:
                prob = score_model(input=curr)
                word_index = self.enc.encode(word)
                probability += np.log(prob[word_index[0]])
                curr = curr + " " + word
            POS = template[0][i]
            results.append((curr, probability))
            print(results)
            return
