...
for e, d in zip(Rhetoric.RHETORICAL_FIGURES, Rhetoric.FIGURE_DESCRIPTION):
    ...
    for id in fig_ids:
        ...
        if (recursive_len(orig_text) > 1):
            ...
            fig_info = Rhetoric.FigureInfo(unique_id, [[sent_table.iloc[i]['text']] for i in sentence_ids],
                                           pos_templates,
                                           rep_templates)
...
def process_gen(e,d):
    ...
    out_name_verbose = 'data/rhet/output/' + corpus + "_" + e.name.lower() + "_" + "verbose" + "_" + str(timestamp) \
                       + "_" + str(os.getpid()) + "_" + str(platform.node()) + ".txt"
    out_name_sparse = 'data/rhet/output/' + corpus + "_" + e.name.lower() + "_" + "sparse" + "_" + str(timestamp) + \
                      "_" + str(os.getpid()) + "_" + str(platform.node()) + ".txt"
    f_verbose = open(out_name_verbose, "w+")
    f_sparse = open(out_name_sparse, "w+")
    ...
    # print(len(filtered))
    f_verbose.write(fig_and_desc + " \n")
    f_verbose.write("NUM TEMMPLATES: %d\n" % (len(filtered)))
    f_verbose.write("BEAM WIDTH: %d \n" % (n))
    f_verbose.write("TOP n=%d SENTENCES \n" % (top_sent))
    f_verbose.write("---------------------------------------------------------------------\n")
    f_verbose.write("---------------------------------------------------------------------\n\n")
    f_verbose.flush()

    f_sparse.write(fig_and_desc + " \n")
    f_sparse.write("NUM TEMMPLATES: %d\n" % (len(filtered)))
    f_sparse.write("BEAM WIDTH: %d \n" % (n))
    f_sparse.write("TOP n=%d SENTENCES \n" % (top_sent))
    f_sparse.write("---------------------------------------------------------------------\n")
    f_sparse.write("---------------------------------------------------------------------\n\n")
    f_sparse.flush()
