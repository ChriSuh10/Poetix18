def bash_generator():
	type="original"
	saved_directory="final_testing_half_diversity"
	search_space=20
	retain_space=30
	...
	with open("run_Limericks_"+saved_directory+"_"+str(cuda)+".sh","w") as f:
		...
		for prompt in prompt_list:
			sentence="CUDA_VISIBLE_DEVICES="+str(cuda)+"   python3 run_Limericks.py"+" --type "+str(type)+" --saved_directory "+str(saved_directory)+" --mode "+str(mode)+" --diversity "+ str(diversity)+" --half_diversity "+ str(half_diversity) + " --cuda "+str(cuda)+" --word_embedding_coefficient "+str(word_embedding_coefficient)+" --search_space "+str(search_space)+" --retain_space "+str(retain_space)+" --prompt "+ str(prompt)+"\n"
			f.write(sentence)
